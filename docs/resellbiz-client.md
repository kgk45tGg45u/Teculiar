...
`npm --workspace @teculiar/api run resellbiz` is a temporary admin script for direct Resell.biz domain work.
...
```bash
npm --workspace @teculiar/api run resellbiz -- status example.com
```

Check which `.env` file and non-secret settings the script sees:

```bash
npm --workspace @teculiar/api run resellbiz -- env-check
```

Show only the transfer code / domain secret:

```bash
npm --workspace @teculiar/api run resellbiz -- auth-code example.com
npm --workspace @teculiar/api run resellbiz -- auth-code 123456789
```

Change nameservers:

```bash
npm --workspace @teculiar/api run resellbiz -- change-ns example.com ns1.host.test ns2.host.test
```

Check if transfer can be requested:

```bash
npm --workspace @teculiar/api run resellbiz -- validate-transfer example.com
```

Start a transfer:

```bash
npm --workspace @teculiar/api run resellbiz -- transfer example.com \
  --customer-id 1000 \
  --reg-contact-id 2000 \
