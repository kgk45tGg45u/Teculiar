...
import { fetchUnreadImapMessages, isAutoGeneratedEmail, type ImapMailboxConfig, type ImapMessage } from "./imap-mailbox";
...
export class TicketsService {
  ...
  // Admin-only hard delete (spam). DB cascade removes replies, notes and attachments.
  async deleteTicket(ticketId: string) {
    const ticket = await this.tickets.findTicket(ticketId);
    if (!ticket) {
      throw new NotFoundException("Ticket not found");
    }
    await this.tickets.deleteTicket(ticket.id);
    return { deleted: true, id: ticket.id };
  }
  ...
  async importMailboxTickets(settings: Record<string, unknown>) {
    ...
    const configs = mailboxConfigs(settings);
    // Our own outbound identities + our own inbox addresses: mail from any of these is the
    // system talking to itself (notification loops), never a customer ticket.
    const systemAddresses = new Set([
      ...(await this.emails?.systemSenderAddresses().catch(() => [] as string[]) ?? []),
      ...configs.map((config) => config.address.trim().toLowerCase())
    ]);

    for (const mailbox of configs) {
      if (!mailbox.enabled) {
      ...
      for (const message of messages) {
        ...
        // Never import machine-generated mail (auto-replies, bounces) or the system's own
        // notifications — they used to loop back in as fresh "customer" tickets.
        if (systemAddresses.has(sender) || isAutoGeneratedEmail(message.headers)) {
          skipped += 1;
          boxSkipped += 1;
          await this.logInbound(mailbox, message, undefined, "SKIPPED_SYSTEM");
          continue;
        }
