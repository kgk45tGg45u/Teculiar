...
import { resolveVat, type TaxContext, type TaxCountryConfig } from "@teculiar/shared";

@Injectable()
export class TaxService {
  // VAT is resolved from the per-country config (single source of truth shared with the web
  // checkout), then EU reverse-charge / non-EU export rules on top. See @teculiar/shared/tax.
